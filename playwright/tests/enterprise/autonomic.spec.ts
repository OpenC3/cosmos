...
test('manually run a reaction', async ({ page, utils }) => {
  ...
  await expect(page.locator('[data-test="new-reaction"]')).toBeEnabled()
  await expect(
    page.getByText('DEFAULT__OPENC3__REACTION was deployed'),
  ).toBeVisible({ timeout: 30000 }) // Takes a while to deploy in Github actions
...
test('enable and disable a reaction', async ({ page, utils }) => {
  await page.getByRole('tab', { name: 'Reactions' }).click()
  await expect(page).toHaveURL(
    'http://localhost:2900/tools/autonomic/reactions',
  )
...
test('enable and disable a trigger', async ({ page, utils }) => {
...
test('create item state trigger', async ({ page, utils }) => {
  ...
  await expect(page.locator('[data-test="log-messages"]')).toContainText(
    ...
    { timeout: 120000 }, // Takes 1:15 to full cycle but worst case is about 1:10 for outside RED_HIGH
  )
