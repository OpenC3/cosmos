...
module OpenC3
  class LogMicroservice < Microservice
    ...
    def log_data(topic, msg_id, msg_hash, redis)
      ...
      @plws[target_name][rt_or_stored].buffered_write(packet_type, @cmd_or_tlm, target_name, packet_name, msg_hash["time"].to_i, rt_or_stored == :STORED, msg_hash[data_key], nil, topic, msg_id, received_time_nsec_since_epoch: msg_hash['received_time'].to_i, extra: msg_hash['extra'])
    rescue => err
      @error = err
