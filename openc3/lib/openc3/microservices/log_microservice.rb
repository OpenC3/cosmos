...
module OpenC3
  class LogMicroservice < Microservice
    ...
    def initialize(name)
      ...
      @target_shard = @target_names.length > 0 ? Store.shard_for_target(@target_names[0], scope: @scope) : 0
    end
    ...
    def run
      ...
      while true
        ...
        if @target_shard != 0
          Topic.read_topics([@microservice_topic], nil, 0) do |topic, msg_id, msg_hash, redis|
