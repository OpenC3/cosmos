...
module OpenC3
  ...
  class QueueInputError < QueueError; end

  class QueueModel < Model
    PRIMARY_KEY = '__openc3__queue'.freeze
    SCRIPT_QUEUE = 'script'.freeze
    COMMAND_QUEUE = 'command'.freeze
    NOTIFY_QUEUE = 'notify'.freeze
    ACTION_TYPES = [SCRIPT_QUEUE, COMMAND_QUEUE, NOTIFY_QUEUE]

    def self.create_unique_name(scope:)
      queue_names = self.names(scope: scope) # comes back sorted
      num = 1 # Users count with 1
      if queue_names[-1]
        num = queue_names[-1][5..-1].to_i + 1
      end
      return "QUEUE#{num}"
    end

    # @return [QueueModel] Return the object with the name at
    ...
    def self.delete(name:, scope:)
      ...
      if model.nil?
        raise QueueInputError.new "queue '#{name}' does not exist"
      end
      ...
      # No notification as this is only called via queue_controller which already notifies

      # undeploy only actually runs if no queues are left
      model.undeploy()
    end
    ...
    def initialize(
      ...
      @microservice_name = "#{scope}__OPENC3__QUEUE"
      @enabled = enabled
    ...
    # Modifiers for the queue_controller update action
    ...
    def validate_level(level)
      ...
      when 'EDGE', 'LEVEL'
        ...
        raise QueueInputError.new "invalid trigger level, must be EDGE or LEVEL: #{level}"
      end
    end
    ...
    def validate_snooze(snooze)
      ...
      raise QueueInputError.new "invalid snooze value: #{snooze}"
    end

    def validate_triggers(triggers)
      unless triggers.is_a?(Array)
        raise QueueInputError.new "invalid triggers, must be array of hashes: #{triggers}"
      end
      trigger_hash = Hash.new()
      triggers.each do | trigger |
        unless trigger.is_a?(Hash)
          raise QueueInputError.new "invalid trigger, must be hash: #{trigger}"
        end
        if trigger['name'].nil? || trigger['group'].nil?
          raise QueueInputError.new "invalid trigger, must contain 'name' and 'group' keys: #{trigger}"
        end
        trigger_name = trigger['name']
        unless trigger_hash[trigger_name].nil?
          raise QueueInputError.new "no duplicate triggers allowed: #{triggers}"
        else
    ...
    def validate_actions(actions)
      unless actions.is_a?(Array)
        raise QueueInputError.new "invalid actions, must be array of hashes: #{actions}"
      end
      actions.each do | action |
        unless action.is_a?(Hash)
          raise QueueInputError.new "invalid action, must be a hash: #{action}"
        end
        action_type = action['type']
        if action_type.nil?
          raise QueueInputError.new "invalid action, must contain 'type': #{action}"
        elsif action['value'].nil?
          raise QueueInputError.new "invalid action, must contain 'value': #{action}"
        end
        unless ACTION_TYPES.include?(action_type)
          raise QueueInputError.new "invalid action type '#{action_type}', must be one of #{ACTION_TYPES}"
        end
      end
      return actions
    end
    ...
    def verify_triggers
      ...
      @triggers.each do | trigger |
        ...
        if model.nil?
          raise QueueInputError.new "failed to find trigger: #{trigger}"
        end
        trigger_models << model
      end
      if trigger_models.empty?
        raise QueueInputError.new "queue must contain at least one valid trigger: #{@triggers}"
      end
    ...
    def create
      unless Store.hget(@primary_key, @name).nil?
        raise QueueInputError.new "existing queue found: #{@name}"
      end
      verify_triggers()
    ...
    def notify_enable
      @enabled = true
      notify(kind: 'enabled')
      # update() will be called by the queue_microservice
    end
    ...
    # @return [QueueModel] Model generated from the passed JSON
    ...
    def undeploy
      return unless QueueModel.names(scope: @scope).empty?
