...
module OpenC3
  ...
  class PythonPackageModel
    ...
    def self.install(name_or_path, scope:)
      ...
      result = OpenC3::ProcessManager.instance.spawn(["/openc3/bin/pipinstall", "--no-warn-script-location", "-i", pypi_url, "--trusted-host", URI.parse(pypi_url).host, package_file_path], "package_install", package_filename, Time.now + 3600.0, scope: scope)
      return result.name
    end
