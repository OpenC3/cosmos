...
class TestInterfaceMicroservice(unittest.TestCase):
    def setUp(self):
        ...
        model.create
        model.update_store(
            System([target], os.path.join(TEST_DIR, "install", "config", "targets"))
        )
        ...
        for packet_name, packet in System.telemetry.packets("INST"):
    ...
    #     config = im.config
    #     self.assertEqual(config["name"], "DEFAULT__INTERFACE__INST_INT")
    #     interface = im.interface
    #     self.assertEqual(interface.name, "INST_INT")
    #     self.assertEqual(interface.state, "ATTEMPTING")
    #     self.assertEqual(interface.target_names, ["INST"])
    #     self.assertEqual(interface.cmd_target_names, ["INST"])
    #     self.assertEqual(interface.tlm_target_names, ["INST"])
    #     all = InterfaceStatusModel.all(scope="DEFAULT")
    #     self.assertEqual(all["INST_INT"]["name"], "INST_INT")
    ...
    #     self.assertEqual(Thread.list.count - init_threads, 3)

    #     im.shutdown()
    #     time.sleep(0.1)  # Allow threads to exit
    #     self.assertEqual(Thread.list.count, init_threads)
