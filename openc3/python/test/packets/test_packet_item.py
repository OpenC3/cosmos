...
class TestPacketItem(unittest.TestCase):
    ...
    def test_converts_to_a_hash(self):
        ...
        self.assertEqual(item["bit_size"], 32)
        self.assertEqual(item["data_type"], "UINT")
        self.assertEqual(item["endianness"], "BIG_ENDIAN")
        self.assertEqual(item["array_size"], 64)
        self.assertEqual(item["overflow"], "ERROR")
        self.assertEqual(item["format_string"], "%5.1f")
        self.assertIsInstance(item["read_conversion"], dict)
        self.assertIsInstance(item["write_conversion"], dict)
        self.assertEqual(item["id_value"], 10)
        true_hash = {"value": 1, "color": "GREEN", "messages_disabled": True}
        false_hash = {"value": 0, "hazardous": "NO!", "color": "RED"}
        self.assertEqual(item["states"], {"TRUE": true_hash, "FALSE": false_hash})
        self.assertEqual(item["description"], "description")
        self.assertEqual(item["units_full"], "Celsius")
        self.assertEqual(item["units"], "C")
        self.assertEqual(item["default"], 0)
        # range turns into minimum and maximum
        self.assertEqual(item["minimum"], 0)
        self.assertEqual(item["maximum"], 100)
        self.assertTrue(item["required"])
        self.assertFalse(item["limits"]["enabled"])
        # State is actually stored in Redis so it doesn't make sense to return via PacketItemLimits
        self.assertIsInstance(item["limits"]["response"], LimitsResponse)
        self.assertEqual(item["limits"]["persistence_setting"], 1)
        # limits values are broken out by set and individual values
        self.assertEqual(item["limits"]["DEFAULT"]["red_low"], 0)
        self.assertEqual(item["limits"]["DEFAULT"]["yellow_low"], 1)
        self.assertEqual(item["limits"]["DEFAULT"]["yellow_high"], 2)
