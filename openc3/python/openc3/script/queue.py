...
def _make_request(action, verb, uri, scope, data=None):
    ...
    if response is None:
        raise RuntimeError(f"Failed to {action}. No response from server.")
...
def queue_remove(name, index=None, scope: str = OPENC3_SCOPE):
    """Remove a command from the queue at the specified index"""
    data = {}
    if index is not None:
        data['index'] = index
