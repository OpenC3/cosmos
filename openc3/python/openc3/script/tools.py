...
def list_settings():
    return COSMOS.json_rpc_request("list_settings", [])


def get_all_settings():
    return COSMOS.json_rpc_request("get_all_settings", [])


def get_setting(name):
    return COSMOS.json_rpc_request("get_setting", [name])


def get_settings(settings_list):
    return COSMOS.json_rpc_request("get_settings", settings_list)


def save_setting(name, data):
    return COSMOS.json_rpc_request("save_setting", [name, data])


def list_configs(tool):
    return COSMOS.json_rpc_request("list_configs", [tool])


def load_config(tool, name):
    return COSMOS.json_rpc_request("load_config", [tool, name])


def save_config(tool, name, data):
    return COSMOS.json_rpc_request("save_config", [tool, name, data])


def delete_config(tool, name):
    return COSMOS.json_rpc_request("delete_config", [tool, name])
