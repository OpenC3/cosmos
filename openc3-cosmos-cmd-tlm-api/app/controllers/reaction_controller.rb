...
class ReactionController < ApplicationController
  ...
  def create
    ...
    begin
      hash = params.permit(:trigger_level, :snooze, :label, :name, triggers: [:name, :group], actions: [:type, :value]).to_h
  ...
  def update
    ...
    begin
      ...
      hash = params.permit(:snooze, :trigger_level, :label, triggers: [:name, :group], actions: [:type, :value]).to_h
