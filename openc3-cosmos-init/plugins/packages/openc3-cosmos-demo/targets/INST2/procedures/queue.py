...
cmd(
    "INST",
    "MEMLOAD",
    {"DATA": b"\x10\x20\x30\x40\xa0\xb0\xc0\xd0\xe0\xf0"},
    queue="QUEUEAPI",
)
...
cmd(
    "INST ASCIICMD with STRING 'NOOP', BINARY 0xDEADBEEF, ASCII '0xDEADBEEF'",
    queue="QUEUEAPI",
)
...
cmd(
    "INST SETPARAMS with VALUE1 1, VALUE2 1, VALUE3 1, VALUE4 1, VALUE5 1, BIGINT 0",
    queue="QUEUEAPI",
)
...
cmd(
    "INST ABORT", queue="QUEUEAPI"
)  # This will raise an error since the queue is disabled
queue_hold("QUEUEAPI")
wait()
