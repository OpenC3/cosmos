  ...
  <div>
    ...
    <div class="text-caption text-medium-emphasis mt-2 mb-2 ml-4">
      ...
      <template v-else-if="hasServiceFilter">
        Showing {{ filteredServiceCount }} / {{ totalServiceCount }} microservices
      </template>
      <template v-else>
      ...
      <div v-for="microservice in filteredMicroservices" :key="microservice">
        <v-list-item>
          ...
          <template #append>
            ...
            />
            <v-list-item-icon v-if="microservice_status[microservice.name]">
              <v-chip
            ...
            <v-list-item-icon>
              <v-btn
                :aria-label="isRunning(microservice.name) ? 'Restart Microservice' : 'Start Microservice'"
                :icon="isRestarting(microservice.name) ? 'mdi-loading' : isRunning(microservice.name) ? 'mdi-refresh' : 'mdi-play'"
...
export default {
  ...
  methods: {
    ...
    checkOperationCompletion: function () {
      for (const [name, tracking] of Object.entries(this.microserviceOperations)) {
    ...
    startOrRestartMicroservice: function (name) {
      ...
      this.$dialog
        .confirm(`Are you sure you want to ${command.toLowerCase()} microservice: ${name}?`, {
          okText: command,
          cancelText: 'Cancel',
        })
        .then((_dialog) => {
