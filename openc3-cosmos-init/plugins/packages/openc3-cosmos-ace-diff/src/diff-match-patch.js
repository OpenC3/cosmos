...
diff_match_patch.prototype.diff_main = function (
  ...
  // console.log(JSON.stringify(diffs)) // Debug the diffs
...
diff_match_patch.prototype.diff_halfMatch_ = function (text1, text2) {
  ...
  function diff_halfMatchI_(longtext, shorttext, i) {
    ...
    while ((j = shorttext.indexOf(seed, j + 1)) != -1) {
      let prefixLength = dmp.diff_commonPrefix(
        longtext.substring(i),
        shorttext.substring(j),
      )
      let suffixLength = dmp.diff_commonSuffix(
  ...
  let hm1 = diff_halfMatchI_(
    longtext,
    shorttext,
    Math.ceil(longtext.length / 4),
  )
  // Check again based on the third quarter.
  let hm2 = diff_halfMatchI_(
    longtext,
    shorttext,
...
diff_match_patch.prototype.diff_cleanupSemanticLossless = function (diffs) {
  ...
  while (pointer < diffs.length - 1) {
    if (
      ...
      if (commonOffset) {
        // JMT: If the end of the common suffix is a newline then
        // keep the newline because we want to favor trailing newlines
        // to support the common use case of adding a line. Otherwise the
...
diff_match_patch.prototype.diff_cleanupSplitSurrogates = function (diffs) {
  ...
  for (let x = 0; x < diffs.length; x++) {
    let thisDiff = diffs[x]
    let thisTop = thisDiff[1][0]
    let thisEnd = thisDiff[1][thisDiff[1].length - 1]
...
diff_match_patch.prototype.decodeURI = function (text) {
  try {
    ...
    while (i < text.length) {
      ...
      let byte1 = (this.digit16(text[i + 1]) << 4) + this.digit16(text[i + 2])
      if ((byte1 & 0x80) === 0) {
      ...
      let byte2 = (this.digit16(text[i + 4]) << 4) + this.digit16(text[i + 5])
      if ((byte2 & 0xc0) !== 0x80) {
        throw new URIError('URI malformed')
      }
      byte2 = byte2 & 0x3f
      ...
      let byte3 = (this.digit16(text[i + 7]) << 4) + this.digit16(text[i + 8])
      if ((byte3 & 0xc0) !== 0x80) {
        throw new URIError('URI malformed')
      }
      byte3 = byte3 & 0x3f
      ...
      let byte4 = (this.digit16(text[i + 10]) << 4) + this.digit16(text[i + 11])
      if ((byte4 & 0xc0) !== 0x80) {
        throw new URIError('URI malformed')
      }
      byte4 = byte4 & 0x3f
...
diff_match_patch.prototype.patch_splitMax = function (patches) {
  ...
  for (let x = 0; x < patches.length; x++) {
    ...
    while (bigpatch.diffs.length !== 0) {
      ...
      let postcontext = this.diff_text1(bigpatch.diffs).substring(
        0,
        this.Patch_Margin,
      )
      if (postcontext !== '') {
