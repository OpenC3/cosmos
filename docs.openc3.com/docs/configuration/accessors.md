...
The ProtoAccessor is used to read and write [Protocol Buffers](https://protobuf.dev/). It serializes and deserializes data using compiled `.proto` definitions, mapping COSMOS items to protobuf fields via the [KEY](telemetry#key) keyword with dot-notation for nested fields. It is commonly used in conjunction with the [GrpcInterface](interfaces#grpc-interface-enterprise) but can be used with any interface.

| Parameter | Description                                                                              | Required |
| --------- | ---------------------------------------------------------------------------------------- | -------- |
| Filename  | File generated by the protocol buffer compiler (`_pb.rb` for Ruby, `_pb2.py` for Python) | Yes      |
| Class     | Protobuf message class to use when encoding and decoding the buffer                      | Yes      |

The KEY values use dot-notation to reference protobuf fields (e.g. `name`, `user.name`, `photo.created_at.seconds`). This differs from JSON/CBOR accessors which use JSONPath (`$.field`). Note that item bit sizes do not matter with the ProtoAccessor since protobuf uses its own variable-length encoding.

The proto-target plugin includes code generation scripts that read `.proto` files and automatically generate COSMOS command and telemetry definition files with the correct accessor configuration, KEY mappings, and type conversions. See the plugin README for details on the `rake generate` workflow.

#### Protobuf Type Mapping

| Protobuf Type                 | COSMOS Type                     | Bit Size |
| ----------------------------- | ------------------------------- | -------- |
| `bool`                        | BOOL                            | 8        |
| `bytes`                       | BLOCK                           | 8        |
| `string`                      | STRING                          | 8        |
| `double`                      | FLOAT                           | 64       |
| `float`                       | FLOAT                           | 32       |
| `int32`, `sint32`, `sfixed32` | INT                             | 32       |
| `int64`, `sint64`, `sfixed64` | INT                             | 64       |
| `uint32`, `fixed32`           | UINT                            | 32       |
| `uint64`, `fixed64`           | UINT                            | 64       |
| Nested message (singular)     | Flattened with dot-notation KEY | -        |
| Repeated message              | ARRAY                           | 8        |

#### Commands

Using the ProtoAccessor for [command definitions](command) requires the use of [KEY](command#key) to map each parameter to a protobuf field. Commands also use `TEMPLATE ''` (empty string) since protobuf handles encoding. When used with the GrpcInterface, commands use `META GRPC_METHOD` to specify the gRPC method path and `RESPONSE` to indicate which telemetry packet receives the gRPC response.
...
COMMAND PROTO CREATE_USER BIG_ENDIAN "Create a User"
  ...
  TEMPLATE ''
  RESPONSE PROTO USER
  APPEND_PARAMETER NAME 8 STRING ''
    KEY name
  APPEND_PARAMETER DISPLAY_NAME 8 STRING ''
    KEY display_name
  APPEND_PARAMETER EMAIL 8 STRING ''
    KEY email
```
...
Nested protobuf messages are flattened using dot-notation in the KEY. For example, an `UpdateUserRequest` message containing a `User user` field and a `FieldMask mask` field:
...
COMMAND PROTO UPDATE_USER BIG_ENDIAN "Update a User"
  ...
  APPEND_PARAMETER NAME 8 STRING ''
    KEY name
  # Nested User message fields accessed via dot-notation
  APPEND_PARAMETER USER.NAME 8 STRING ''
    KEY user.name
  APPEND_PARAMETER USER.DISPLAY_NAME 8 STRING ''
    KEY user.display_name
  APPEND_PARAMETER USER.EMAIL 8 STRING ''
    KEY user.email
  # Repeated field (FieldMask paths)
  APPEND_ARRAY_PARAMETER MASK.PATHS 8 STRING 8
    KEY mask.paths
```
...
When a gRPC method returns `google.protobuf.Empty`, omit the `RESPONSE` line:
...
Using the ProtoAccessor for [telemetry definitions](telemetry) requires the use of [KEY](telemetry#key) to pull values from the decoded protobuf message. Nested well-known types like `google.protobuf.Timestamp` are flattened into their component fields.
...
TELEMETRY PROTO USER BIG_ENDIAN "User Telemetry"
  ACCESSOR proto_accessor.rb example_photo_pb.rb Example::Photoservice::User
  TEMPLATE ''
  APPEND_ITEM NAME 8 STRING
    KEY name
  APPEND_ITEM DISPLAY_NAME 8 STRING
    KEY display_name
  APPEND_ITEM EMAIL 8 STRING
    KEY email
```
...
TELEMETRY PROTO PHOTO BIG_ENDIAN "Photo Telemetry"
  ...
  APPEND_ITEM DISPLAY_NAME 8 STRING
    KEY display_name
  APPEND_ITEM CREATED_AT.SECONDS 64 INT
    KEY created_at.seconds
  APPEND_ITEM CREATED_AT.NANOS 32 INT
...
TELEMETRY PROTO LIST_PHOTOS_RESPONSE BIG_ENDIAN "List of Photos"
  ...
  TEMPLATE ''
  APPEND_ITEM PHOTOS 8 ARRAY
    KEY photos
  APPEND_ITEM NEXT_PAGE_TOKEN 8 STRING
    KEY next_page_token
```
