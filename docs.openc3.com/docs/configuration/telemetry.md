...
Conversions are implemented in a custom Ruby or Python file which should be located in the target's lib folder. The Ruby class must require 'openc3/conversions/conversion' and inherit from Conversion. The Python class must import Conversion from openc3.conversions.conversion and inherit Conversion. The class must implement the initialize / \_\_init\_\_ method if it takes extra parameters and must always implement the call() method. The conversion factor is applied to the raw value in the telemetry packet before it is displayed to the user. The user still has the ability to see the raw unconverted value in a details dialog.
...
class TheGreatConversion(Conversion):
    def __init__(self, multiplier):
        super().__init__()
        self.multiplier = multiplier.to_f
    def call(value, packet, buffer):
...
| Parameter                 | Description                                                                                                                                           | Required |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | -------- |
| Response Class Filename   | Name of the Ruby or Python file which implements the limits response. This file should be in the target's lib directory so it can be found by OpenC3. | True     |
...
Defined in example_limits_response.py:
