...
<TabItem value="linux" label="Linux">
  ```sh
  cosmos-project % openc3.sh cli generate plugin BOB --python
...
Starting with [COSMOS v5.5.0](https://github.com/OpenC3/cosmos/releases/tag/v5.5.0), the plugin generator creates just the plugin framework (previously it would also create a target). From within the newly created plugin directory, we generate a target.

<Tabs groupId="platform">
<TabItem value="linux" label="Linux">
  ```sh
  cosmos-project % cd openc3-cosmos-bob
  openc3-cosmos-bob % openc3.sh cli generate target BOB --python
...
:::info[Generators]
There are a number of generators available. Run `openc3.sh cli generate` or `openc3.bat cli generate` to see all the available options.
:::

### Command Definition

The target generator creates a single target named BOB. Best practice is to create a single target per plugin to make it easier to share targets and upgrade them individually. Lets see what the target generator created for us. Open the `openc3-cosmos-bob/targets/BOB/cmd_tlm/cmd.txt`:
...
Now open the `openc3-cosmos-bob/targets/BOB/cmd_tlm/tlm.txt`:
...
Now we need to tell COSMOS how to connect to our BOB target. Open the `openc3-cosmos-bob/plugin.txt` file:

```cosmos
# Set VARIABLEs here to allow variation in your plugin
# See [Plugins](../configuration/plugins) for more information
VARIABLE bob_target_name BOB
  VARIABLE_DESCRIPTION "Name for BOB target"
...
Now we need to build our plugin and upload it to COSMOS.

<Tabs groupId="platform">
<TabItem value="linux" label="Linux">
  ```sh
  openc3-cosmos-bob % openc3.sh cli rake build VERSION=1.0.0
...
<TabItem value="linux" label="Linux">
  ```sh
  openc3-cosmos-bob % openc3.sh cli rake build VERSION=1.0.1
    Successfully built RubyGem
    Name: openc3-cosmos-bob
    Version: 1.0.1
