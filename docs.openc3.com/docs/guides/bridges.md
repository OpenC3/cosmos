...
- Requires a host Python 3.10+ installation
- Install the OpenC3 package (recommend using a venv)

```bash
python3 -m venv venv
source venv/bin/activate
python3 -m pip install openc3
```

Note: Developers working from a checkout can use `pip install -e cosmos/openc3/python` instead of `pip install openc3`

</TabItem>
<TabItem value="ruby" label="Ruby">

- Requires a host Ruby 3+ installation
...
Bridges are run using a configuration file named bridge.txt. This file is a subset of the plugin.txt configuration syntax supporting VARIABLE, INTERFACE, ROUTER, and associated modifier keywords. However, BRIDGES HAVE NO KNOWLEDGE OF TARGETS. So instead of MAP_TARGETS, the INTERFACE is associated with the ROUTER using the ROUTE keyword.

To create a bridge configuration file do the following:

<Tabs groupId="script-language">
<TabItem value="python" label="Python">

```bash
openc3pycli bridgesetup
```
...
The following is the default bridge.txt that is generated:
...
Runs a bridge from a given configuration file. Defaults to bridge.txt in the current directory. Variables can also be passed into override VARIABLE defaults.
