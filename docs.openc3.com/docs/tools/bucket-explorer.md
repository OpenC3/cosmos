...
Using bucket storage allows COSMOS to directly serve the tools as a static website and thus it defaults to a public bucket in cloud technology like S3. You can set `toolsBucketPrivate` in the helm charts to use a private tools bucket which routes all requests through the cmd-tlm-api.
:::
