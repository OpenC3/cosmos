...
Most settings on this tab can also be configured at deploy time with an `OPENC3_SETTING_<NAME>` environment variable, so a fresh install comes up already configured instead of requiring a pass through this tab. A setting changed here is then left alone on restart. See [cli initsettings](../getting-started/cli.md#initsettings) for the list of settings that can be seeded.
