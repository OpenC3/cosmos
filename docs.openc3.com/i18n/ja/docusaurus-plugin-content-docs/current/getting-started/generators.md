...
| targets/GSE/procedures                 | このフォルダには、ターゲットの機能を実行するターゲット固有の手順とヘルパーメソッドが含まれています。これらの手順はシンプルに保ち、このターゲットに関連付けられたコマンドとテレメトリの定義のみを使用する必要があります。詳細については、[スクリプト作成ガイド](../guides/script-writing#script-organization)を参照してください。                                                                                                     |
