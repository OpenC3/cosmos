  ...
  describe "GET index" do
    ...
    context "without scope" do
      it "renders status error" do
        ...
        expect(JSON.parse(response.body, symbolize_names: true, :allow_nan => true)).to include(status: 'error')
        expect(JSON.parse(response.body, symbolize_names: true, :allow_nan => true)).to include(message: 'Scope is required')
      end
    end
    ...
    context "with scope" do
      ...
      it "renders error with missing token" do
        ...
        expect(JSON.parse(response.body, symbolize_names: true, :allow_nan => true)).to include(status: 'error')
        expect(JSON.parse(response.body, symbolize_names: true, :allow_nan => true)).to include(message: 'Token is required')
